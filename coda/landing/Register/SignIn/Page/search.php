    ...
    <div class="row">
        <div class="tree">
            ...
            <ul>
                ...
                LIMIT 3"; // Limit the query to fetch only 3 schedules
...
// Query to select faculty name and status
...
if ($result && mysqli_num_rows($result) > 0) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    // Convert status abbreviations to full forms
    switch ($row['status']) {
      case 'ou':
        $status = 'OUT';
        $color = 'red'; 
...
$(document).ready(function() {
  ...
  $(document).on('click', function(event) {
    if (!$(event.target).closest('#searchInput').length) {
      // If the click is not within the search input, hide the suggestions
      $('#searchSuggestions').empty();
    }
  });
});

// Function to send search query to server and display suggestions
function searchSuggestions(query) {
  $.ajax({
    url: 'search.php', // Change this to your PHP file
    method: 'POST',
    data: { query: query },
